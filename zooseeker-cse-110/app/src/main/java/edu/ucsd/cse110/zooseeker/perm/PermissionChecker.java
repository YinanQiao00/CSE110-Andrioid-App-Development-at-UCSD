package edu.ucsd.cse110.zooseeker.perm;
...
public class PermissionChecker {
	...
	public boolean ensureAllPermission() {
		var hasNoLocationPerms = Arrays.stream(requiredPermissions).map(perm -> ContextCompat.checkSelfPermission(activity, perm)).allMatch(status -> status == PackageManager.PERMISSION_DENIED);
